...
public class LedgerPartyActivity extends AppCompatActivity {
    ...
    String GROUP_NAME = "SUNDRY DEBTORS";
...
// Read the response and write it to standard out.
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                ...
                JSONArray ja_data = jsonObject.getJSONArray("DSPACCNAME");
                for(int i=0; i<ja_data.length(); i++) {
                    arrarycashbalencelistdata= new HashMap<String, String>();
                    JSONObject jObj = ja_data.getJSONObject(i);

                    String DSPCLDRAMTA = jsonObject.getJSONArray("DSPACCINFO").getJSONObject(i).getJSONObject("DSPCLDRAMT").
                            getString("DSPCLDRAMTA");
                    String DSPCLCRAMTA = jsonObject.getJSONArray("DSPACCINFO").getJSONObject(i).getJSONObject("DSPCLCRAMT").
                            getString("DSPCLCRAMTA");
//                    String billoverdue = jsonObject.getJSONArray("BILLOVERDUE").getString(i);
                    if(!DSPCLDRAMTA.equals("") && !DSPCLCRAMTA.equals("")) {
                        arrarycashbalencelistdata.put("DSPDISPNAME", jObj.getString("DSPDISPNAME"));
                        arrarycashbalencelistdata.put("BSMAINAMT", "₹" + DSPCLDRAMTA + "\n₹" + DSPCLCRAMTA);
                    } else {
                        if(!DSPCLDRAMTA.equals("")) {
                            arrarycashbalencelistdata.put("DSPDISPNAME", jObj.getString("DSPDISPNAME"));
                            arrarycashbalencelistdata.put("BSMAINAMT", "₹" + DSPCLDRAMTA);
                        }

                        if(!DSPCLCRAMTA.equals("")) {
                            arrarycashbalencelistdata.put("DSPDISPNAME", jObj.getString("DSPDISPNAME"));
                            arrarycashbalencelistdata.put("BSMAINAMT", "₹" + DSPCLCRAMTA);
                        }
                    }
...
*/
                ...
                QuestionsAdapter questionsAdapter=new QuestionsAdapter(LedgerPartyActivity.this,arrarycashbalencelist);
                listview.setAdapter(questionsAdapter);

                listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    @Override
                    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                        String group_name = arrarycashbalencelist.get(i).get("DSPDISPNAME");
                        Intent in = new Intent(LedgerPartyActivity.this, LedgerPartyActivity.class);
